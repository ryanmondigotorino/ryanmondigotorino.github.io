...
const Drawer: React.FC = () => {
  ...
  return (
    <Wrapper.Drawer
        className={drawer?.action ? !drawer?.isVisible ? 'in-active' : 'active' : 'd-none'}
      >
        <div className="drawer-container">
          <Direction.Row className="justify-content-end">
            <ButtonApp.Close type="button" onClick={() => dispatch(toggleDrawer({ isVisible: false,  action: 'close' }))}>
              <IconClose className="icon icon-close" />
            </ButtonApp.Close>
          </Direction.Row>
          <Text.Title className="mb-20">Get in Touch</Text.Title>
          <form onSubmit={(e: React.FormEvent<HTMLFormElement>) => onSubmit(e as unknown as React.FormEvent<HTMLInputElement>)}>
